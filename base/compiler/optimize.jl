...
struct InlineeInfo
    rt
    mi::MethodInstance
    InlineeInfo(@nospecialize(rt), mi::MethodInstance) = new(rt, mi)
end

"""
    is_inlineable(interp::AbstractInterpreter, src::MaybeCompressed,
                  metainfo::Union{InlineeInfo,Nothing}=nothing) -> Bool
...
If `metainfo::InlineeInfo` is available, take it into account to add extra inlining willingness.
"""
function is_inlineable(interp::AbstractInterpreter, @nospecialize(src::MaybeCompressed),
                       metainfo::Union{InlineeInfo,Nothing}=nothing)
    ...
    if metainfo !== nothing
        ...
        if ⊑(ipo_lattice(interp), metainfo.rt, Tuple) && !isconcretetype(metainfo.rt)
...
struct InlineCallInfo
    ...
    info::CallInfo
    stmt_flag::UInt8
    function InlineCallInfo(@nospecialize(rt), mi::MethodInstance, argtypes::Vector{Any},
...
"""
    inlining_policy(interp::AbstractInterpreter, src, iinfo::InlineCallInfo) -> newsrc
...
"""
function inlining_policy(interp::AbstractInterpreter, @nospecialize(src), iinfo::InlineCallInfo)
    if isa(src, MaybeCompressed)
        if is_source_inferred(src)
            if is_stmt_inline(iinfo.stmt_flag) || is_inlineable(interp, src, InlineeInfo(iinfo.rt, iinfo.mi))
                return src
            end
        end
